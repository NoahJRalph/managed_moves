...
<p><strong>UNPACKING/SET UP</strong>: Specific client requests for how the leave the home</p>"`
      ...
      // Client's Full Name for the first part of the CSV file name.
      $('#masterContainer').find('input[name="clientFullName"]').val() + '_TeamUp.csv', 
      {type: 'text/csv'}
    );
    // Pass the CSV file to the download function.
    downloadFile(fileCSV);

    // #2: Excel File Blueprint
